...
class BiDiscriminatorLoss(BiGANLoss):
    ...
    def forward(self, output):
        ...
        # print("its loss without Sx and Sz")

        #loss without Sx and Sz
        # real_output = torch.mean(F.relu(1. - comb_output_real))
        # fake_output = torch.mean(F.relu(1. + comb_output_fake))
        # print("its loss without Sx and Sz")
...
class GeneratorEncoderLoss(BiGANLoss):
    ...
    def forward(self, output):
        ...
        # #loss without Sz
        # real_output = torch.mean(img_output_fake + comb_output_real)
        # fake_output = torch.mean(img_output_fake + comb_output_fake)

        #loss without both of them
        # real_output = torch.mean(comb_output_real)
        # fake_output = torch.mean(comb_output_fake)
